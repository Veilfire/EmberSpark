...
function parseField(field: string): number[] {
  ...
  for (const p of pieces) {
    if (p === "*") {
      ...
      for (let i = 0; i <= 59; i++) out.add(i); // minutes
      for (let i = 0; i <= 23; i++) out.add(i); // hours
      for (let i = 1; i <= 31; i++) out.add(i); // day
      for (let i = 1; i <= 12; i++) out.add(i); // month
      for (let i = 0; i <= 6; i++) out.add(i); // weekday
      // The checks below use field-specific ranges; this is fine.
    } else if (p.startsWith("*/")) {
      const step = parseInt(p.slice(2), 10);
      if (step > 0) {
        for (let i = 0; i <= 59; i++) if (i % step === 0) out.add(i);
      }
    } else if (p.includes("-")) {
...
export function CronPreview({ expr }: { expr: string }) {
  ...
  return (
    <div className="text-xs text-spark-muted">
      Next fires:{" "}
      <span className="text-spark-text">
        {times
          .map((t) =>
