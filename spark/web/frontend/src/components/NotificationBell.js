...
function NotificationRow({ notification, onRead, onDismiss, onNavigate, }) {
    ...
    return (_jsx("li", { className: cn("p-3 hover:bg-spark-border/30 flex flex-col gap-1", severityClass, unread && "border-l-2 border-spark-accent"), children: _jsxs("div", { className: "flex items-start justify-between gap-2", children: [_jsxs("div", { className: "flex-1 min-w-0", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx("span", { className: "text-xs uppercase tracking-wide text-spark-muted", children: notification.kind.replace(/_/g, " ") }), _jsx("span", { className: "text-xs text-spark-muted", children: formatTimestamp(notification.created_at) })] }), actionHref ? (_jsx(NavLink, { to: actionHref, onClick: () => {
                                onRead();
                                onNavigate();
