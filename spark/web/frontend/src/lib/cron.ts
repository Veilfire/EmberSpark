...
export interface CronFields {
  /** 1-N for ``every_minutes`` / ``every_hours``. */
  every?: number;
  /** 0–23 (HH) for daily / weekly / monthly / yearly / every_weekday. */
  hour?: number;
  /** 0–59 (MM) for the same set. */
  minute?: number;
  /** Selected days of week for ``weekly``. Lowercase short names. */
  weekdays?: WeekdayName[];
  /** 1–31 day of month for ``monthly`` / ``yearly``. */
  day?: number;
  /** 1–12 for ``yearly``. */
  month?: number;
  /** Verbatim cron expression for ``custom`` mode. */
  custom?: string;
}
...
export function weekdayLabel(d: WeekdayName): string {
...
export function parseTimeOfDay(s: string): { hour: number; minute: number } | null {
...
function parseDow(field: string): WeekdayName[] | null {
  if (field === "*" || field === "") return null;
  const numericMap: Record<string, WeekdayName> = {
  ...
  for (const p of parts) {
    ...
    } else if ((WEEKDAY_NAMES as readonly string[]).includes(p)) {
      out.push(p as WeekdayName);
    } else {
...
function isWeekdayRange1to5(field: string): boolean {
...
export function parseCron(expr: string): ParseResult {
  ...
  if (mo === "*" && d === "*" && dow === "*" && h === "*") {
    ...
    if (step) {
      ...
      if (n >= 1 && n <= 59) return { preset: "every_minutes", fields: { every: n } };
    }
  }
  ...
  if (mo === "*" && d === "*" && dow === "*" && isInt(m)) {
    ...
    if (step) {
      const n = parseInt(step[1], 10);
      if (n >= 1 && n <= 23) return { preset: "every_hours", fields: { every: n } };
    }
  }
 ...
 */
export function buildCron(preset: CronPreset, fields: CronFields): string {
  switch (preset) {
    case "every_minutes": {
      const n = fields.every ?? 0;
      if (n < 1 || n > 59) throw new Error("every_minutes N must be 1..59");
      return `*/${n} * * * *`;
    }
    case "every_hours": {
      const n = fields.every ?? 0;
      if (n < 1 || n > 23) throw new Error("every_hours N must be 1..23");
      return `0 */${n} * * *`;
    }
    case "daily": {
    ...
    case "monthly": {
      ...
      if (d < 1 || d > 31) throw new Error("monthly day must be 1..31");
      return `${m} ${h} ${d} * *`;
    }
    case "yearly": {
      const h = fields.hour ?? 0;
      ...
      if (d < 1 || d > 31) throw new Error("yearly day must be 1..31");
      if (mo < 1 || mo > 12) throw new Error("yearly month must be 1..12");
      return `${m} ${h} ${d} ${mo} *`;
    }
    case "custom": {
      const c = (fields.custom ?? "").trim();
      if (!c) throw new Error("custom: expression is empty");
