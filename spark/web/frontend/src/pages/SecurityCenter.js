...
const LEVELS = ["allow", "warn", "redact", "shadow_block", "block"];
function DataClassesPanel() {
    const classes = useQuery({
        queryKey: ["data-classes"],
        queryFn: () => api.get("/api/security/data-classes"),
    });
    const policy = useQuery({
        queryKey: ["data-policy"],
        queryFn: () => api.get("/api/security/data-policy"),
    });
    const grants = useQuery({
    ...
    const detections = useQuery({
        ...
        queryFn: () => api.get("/api/security/data-detections?hours=24"),
        refetchInterval: 30_000,
    });
    return (_jsxs("div", { className: "space-y-6", children: [_jsxs("div", { className: "panel p-4", children: [_jsx("h3", { className: "font-semibold mb-2", children: "Data Classification Guardrails" }), _jsxs("p", { className: "text-xs text-spark-muted", children: ["Name-based detectors for PII, financial, credentials, and dangerous-CLI patterns. Resolution order:", " ", _jsx("code", { className: "font-mono", children: "grant" }), " \u2192", " ", _jsx("code", { className: "font-mono", children: "agent override" }), " \u2192", " ", _jsx("code", { className: "font-mono", children: "global" }), " \u2192", " ", _jsx("code", { className: "font-mono", children: "built-in default" }), ". Levels:", " ", _jsx("code", { className: "font-mono", children: "allow" }), ",", " ", _jsx("code", { className: "font-mono", children: "warn" }), ",", " ", _jsx("code", { className: "font-mono", children: "redact" }), ",", " ", _jsx("code", { className: "font-mono", children: "block" }), "."] })] }), _jsx(GlobalPolicyMatrix, { classes: classes.data ?? [], policy: policy.data }), _jsx(AgentPolicyOverrides, { classes: classes.data ?? [], policy: policy.data }), _jsx(DataGrantsSection, { classes: classes.data ?? [], grants: grants.data ?? [] }), _jsx(DetectionRollup, { rollup: detections.data })] }));
}
...
function GlobalPolicyMatrix({ classes, policy, }) {
    const qc = useQueryClient();
    const [editing, setEditing] = useState(null);
    const byClass = {};
    for (const row of policy?.global ?? [])
        byClass[row.data_class] = row;
    return (_jsxs("div", { className: "panel p-4", children: [_jsx("h3", { className: "font-semibold mb-3", children: "Global policy" }), _jsxs("table", { className: "w-full text-sm", children: [_jsx("thead", { className: "text-left text-xs uppercase tracking-wide text-spark-muted", children: _jsxs("tr", { children: [_jsx("th", { className: "py-1", children: "Class" }), _jsx("th", { children: "Level" }), _jsx("th", { children: "Scopes" }), _jsx("th", { children: "Source" }), _jsx("th", {})] }) }), _jsx("tbody", { children: classes.map((c) => {
                            ...
                            return (_jsxs("tr", { className: "border-t border-spark-border", children: [_jsxs("td", { className: "py-2", children: [_jsx("div", { className: "font-mono", children: c.data_class }), _jsx("div", { className: "text-xs text-spark-muted", children: c.description })] }), _jsx("td", { children: _jsx("span", { className: levelChip(level), children: levelLabel(level) }) }), _jsx("td", { children: _jsx("div", { className: "flex flex-wrap gap-1", children: scopes.map((s) => (_jsx("span", { className: "chip text-xs font-mono", children: s }, s))) }) }), _jsx("td", { className: "text-xs text-spark-muted", children: override ? "override" : "default" }), _jsx("td", { className: "text-right", children: _jsx("button", { className: "btn text-xs", onClick: () => setEditing(c), children: "Edit" }) })] }, c.data_class));
                        }) })] }), editing && (_jsx(PolicyEditor, { cls: editing, current: byClass[editing.data_class], onClose: () => setEditing(null), onSave: async (level, scopes, reason) => {
                    if (level === "block") {
                        const ok = await confirmDialog({
                            title: `Block ${editing.data_class} globally?`,
                            description: "A block level aborts any operation that contains the matching content. This applies to every agent that doesn't have an override. Audited at elevated severity.",
                            tone: "warning",
                    ...
                    await api.put(`/api/security/data-policy/global/${encodeURIComponent(editing.data_class)}`, { level, scopes, reason });
                    toast.success("Global policy updated");
                    qc.invalidateQueries({ queryKey: ["data-policy"] });
                    setEditing(null);
                } }))] }));
}
function AgentPolicyOverrides({ classes, policy, }) {
    const qc = useQueryClient();
    const [agent, setAgent] = useState("");
    const [editing, setEditing] = useState(null);
    const agents = useQuery({
    ...
    const overrides = agent ? policy?.agents?.[agent] ?? [] : [];
    const byClass = {};
    for (const r of overrides)
        byClass[r.data_class] = r;
    return (_jsxs("div", { className: "panel p-4", children: [_jsx("h3", { className: "font-semibold mb-3", children: "Per-agent overrides" }), _jsxs("div", { className: "flex items-center gap-2 mb-3", children: [_jsx("span", { className: "label", children: "Agent:" }), _jsxs("select", { className: "input", value: agent, onChange: (e) => setAgent(e.target.value), children: [_jsx("option", { value: "", children: "(select)" }), (agents.data ?? []).map((a) => (_jsx("option", { value: a.name, children: a.name }, a.name)))] })] }), !agent && (_jsx("p", { className: "text-xs text-spark-muted", children: "Pick an agent to view or set per-agent overrides. Without an override, the agent inherits the global policy above." })), agent && (_jsxs("table", { className: "w-full text-sm", children: [_jsx("thead", { className: "text-left text-xs uppercase tracking-wide text-spark-muted", children: _jsxs("tr", { children: [_jsx("th", { className: "py-1", children: "Class" }), _jsx("th", { children: "Effective level" }), _jsx("th", { children: "Source" }), _jsx("th", {})] }) }), _jsx("tbody", { children: classes.map((c) => {
                            const override = byClass[c.data_class];
                            const level = override?.level ?? c.default_level;
                            return (_jsxs("tr", { className: "border-t border-spark-border", children: [_jsx("td", { className: "py-2 font-mono", children: c.data_class }), _jsx("td", { children: _jsx("span", { className: levelChip(level), children: levelLabel(level) }) }), _jsx("td", { className: "text-xs text-spark-muted", children: override ? "agent override" : "inherits global/default" }), _jsxs("td", { className: "text-right space-x-1", children: [_jsx("button", { className: "btn text-xs", onClick: () => setEditing(c), children: "Set override" }), override && (_jsx("button", { className: "btn btn-danger text-xs", onClick: async () => {
                                                    const ok = await confirmDialog({
                                                        title: `Revert ${agent} → ${c.data_class}?`,
                                                        description: "Removes the agent-specific override so the class falls back to the global policy (or its built-in default).",
                                                        tone: "default",
                                                        confirmLabel: "Revert",
                                                    });
                                                    if (!ok)
                                                        return;
                                                    await api.del(`/api/security/data-policy/agent/${encodeURIComponent(agent)}/${encodeURIComponent(c.data_class)}`);
                                                    toast.success("Override removed");
                                                    qc.invalidateQueries({ queryKey: ["data-policy"] });
                                                }, children: "Revert" }))] })] }, c.data_class));
                        }) })] })), editing && agent && (_jsx(PolicyEditor, { cls: editing, current: byClass[editing.data_class], onClose: () => setEditing(null), onSave: async (level, scopes, reason) => {
                    await api.put(`/api/security/data-policy/agent/${encodeURIComponent(agent)}/${encodeURIComponent(editing.data_class)}`, { level, scopes, reason });
                    toast.success("Agent override saved");
                    qc.invalidateQueries({ queryKey: ["data-policy"] });
                    setEditing(null);
                } }))] }));
}
function PolicyEditor({ cls, current, onClose, onSave, }) {
    const [level, setLevel] = useState(current?.level ?? cls.default_level);
    const [scopes, setScopes] = useState(current?.scopes ?? cls.default_scopes);
    const [reason, setReason] = useState(current?.reason ?? "");
    const [saving, setSaving] = useState(false);
    const toggleScope = (s) => {
        setScopes((prev) => prev.includes(s) ? prev.filter((x) => x !== s) : [...prev, s]);
    };
    return (_jsxs("div", { className: "mt-4 panel p-4 border border-spark-accent/40 space-y-3", children: [_jsxs("div", { children: [_jsx("div", { className: "font-mono", children: cls.data_class }), _jsx("div", { className: "text-xs text-spark-muted", children: cls.description })] }), _jsxs("div", { children: [_jsx("label", { className: "label", children: "Level" }), _jsx("div", { className: "flex gap-2 mt-1 text-sm flex-wrap", children: LEVELS.map((l) => (_jsxs("label", { className: "flex items-center gap-1 cursor-pointer", children: [_jsx("input", { type: "radio", name: "level", checked: level === l, onChange: () => setLevel(l) }), _jsx("span", { className: levelChip(l), children: levelLabel(l) })] }, l))) }), level === "shadow_block" && (_jsxs("p", { className: "text-xs text-spark-muted mt-2", children: [_jsx("strong", { children: "Shadow mode:" }), " audit each hit as if the class were blocked, but let the operation through. Use to measure false-positive rate on real traffic before committing to", " ", _jsx("code", { className: "font-mono", children: "block" }), ". Events land in the audit log under ", _jsx("code", { className: "font-mono", children: "security.data_class.shadow_block" }), "."] }))] }), _jsxs("div", { children: [_jsx("label", { className: "label", children: "Scopes" }), _jsx("div", { className: "flex flex-wrap gap-2 mt-1 text-sm", children: ALL_SCOPES.map((s) => (_jsxs("label", { className: "flex items-center gap-1 cursor-pointer", children: [_jsx("input", { type: "checkbox", checked: scopes.includes(s), onChange: () => toggleScope(s) }), _jsx("span", { className: "font-mono text-xs", children: s })] }, s))) })] }), _jsxs("label", { className: "block", children: [_jsx("span", { className: "label", children: "Reason (audited)" }), _jsx("input", { className: "input w-full", value: reason, onChange: (e) => setReason(e.target.value), placeholder: "why this level?" })] }), _jsxs("div", { className: "flex justify-end gap-2", children: [_jsx("button", { className: "btn", onClick: onClose, children: "Cancel" }), _jsx("button", { className: "btn btn-primary", disabled: saving || scopes.length === 0, onClick: async () => {
                            setSaving(true);
                            try {
...
function DataGrantsSection({ classes, grants, }) {
    const qc = useQueryClient();
    const [creating, setCreating] = useState(false);
    return (_jsxs("div", { className: "panel p-4", children: [_jsxs("div", { className: "flex items-center justify-between mb-3", children: [_jsx("h3", { className: "font-semibold", children: "Unlimited grants" }), _jsx("button", { className: "btn btn-primary text-xs", onClick: () => setCreating(true), children: "+ New grant" })] }), _jsxs("p", { className: "text-xs text-spark-muted mb-3", children: ["Explicit carve-outs that bypass the policy hierarchy. Use when an agent's job is to handle a given class \u2014 e.g. a CC-processing agent gets a ", _jsx("code", { className: "font-mono", children: "financial.card" }), " ", "grant with ", _jsx("code", { className: "font-mono", children: "level_override=allow" }), ". Creation is audited at critical severity."] }), grants.length === 0 ? (_jsx("p", { className: "text-sm text-spark-muted", children: "No active grants." })) : (_jsxs("table", { className: "w-full text-sm", children: [_jsx("thead", { className: "text-left text-xs uppercase tracking-wide text-spark-muted", children: _jsxs("tr", { children: [_jsx("th", { className: "py-1", children: "Agent" }), _jsx("th", { children: "Class" }), _jsx("th", { children: "Scopes" }), _jsx("th", { children: "Level" }), _jsx("th", { children: "Reason" }), _jsx("th", { children: "Expires" }), _jsx("th", {})] }) }), _jsx("tbody", { children: grants.map((g) => (_jsxs("tr", { className: "border-t border-spark-border", children: [_jsx("td", { className: "py-1 font-mono", children: g.agent_name }), _jsx("td", { className: "font-mono", children: g.data_class }), _jsx("td", { children: _jsx("div", { className: "flex flex-wrap gap-1", children: g.scopes.map((s) => (_jsx("span", { className: "chip text-xs font-mono", children: s }, s))) }) }), _jsx("td", { children: _jsx("span", { className: levelChip(g.level_override), children: g.level_override }) }), _jsx("td", { className: "text-xs text-spark-muted", children: g.reason }), _jsx("td", { className: "text-xs", children: formatUntil(g.expires_at) }), _jsx("td", { className: "text-right", children: _jsx("button", { className: "btn btn-danger text-xs", onClick: async () => {
                                            const ok = await confirmDialog({
                                        ...
                                        }, children: "Revoke" }) })] }, g.id))) })] })), creating && (_jsx(GrantCreateForm, { classes: classes, onClose: () => setCreating(false), onCreated: () => {
                    qc.invalidateQueries({ queryKey: ["data-grants"] });
                    setCreating(false);
                } }))] }));
}
function GrantCreateForm({ classes, onClose, onCreated, }) {
    const agents = useQuery({
        queryKey: ["agents"],
        queryFn: () => api.get("/api/scheduler/agents"),
    });
    ...
    async function submit() {
        ...
        if (ttlHours === null) {
            const ok = await confirmDialog({
                ...
                description: "Permanent grants never expire. Prefer a TTL unless you have a concrete reason to make this carve-out forever. Audited at critical severity.",
                tone: "danger",
    ...
    return (_jsxs("div", { className: "mt-4 panel p-4 border border-spark-accent/40 space-y-3", children: [_jsx("h4", { className: "font-semibold", children: "New data-class grant" }), _jsxs("div", { className: "grid grid-cols-2 gap-3", children: [_jsxs("label", { className: "block", children: [_jsx("span", { className: "label", children: "Agent" }), _jsxs("select", { className: "input w-full", value: agent, onChange: (e) => setAgent(e.target.value), children: [_jsx("option", { value: "", children: "(select)" }), (agents.data ?? []).map((a) => (_jsx("option", { value: a.name, children: a.name }, a.name)))] })] }), _jsxs("label", { className: "block", children: [_jsx("span", { className: "label", children: "Data class" }), _jsxs("select", { className: "input w-full", value: dataClass, onChange: (e) => setDataClass(e.target.value), children: [_jsx("option", { value: "", children: "(select)" }), classes.map((c) => (_jsx("option", { value: c.data_class, children: c.data_class }, c.data_class)))] })] })] }), _jsxs("div", { children: [_jsx("span", { className: "label", children: "Scopes" }), _jsx("div", { className: "flex flex-wrap gap-2 mt-1 text-sm", children: ALL_SCOPES.map((s) => (_jsxs("label", { className: "flex items-center gap-1 cursor-pointer", children: [_jsx("input", { type: "checkbox", checked: scopes.includes(s), onChange: () => toggleScope(s) }), _jsx("span", { className: "font-mono text-xs", children: s })] }, s))) })] }), _jsxs("label", { className: "block", children: [_jsx("span", { className: "label", children: "Reason (audited)" }), _jsx("input", { className: "input w-full", value: reason, onChange: (e) => setReason(e.target.value), placeholder: "why this agent needs unrestricted access" })] }), _jsxs("div", { className: "grid grid-cols-2 gap-3 items-end", children: [_jsxs("label", { className: "block", children: [_jsx("span", { className: "label", children: "TTL (hours)" }), _jsx("input", { className: "input w-full", type: "number", min: 1, max: 720, value: ttlHours ?? "", disabled: ttlHours === null, onChange: (e) => setTtlHours(e.target.value ? Number(e.target.value) : 168) })] }), _jsxs("label", { className: "flex items-center gap-2 text-sm cursor-pointer pb-2", children: [_jsx("input", { type: "checkbox", checked: ttlHours === null, onChange: (e) => setTtlHours(e.target.checked ? null : 168) }), "Permanent (no TTL)"] })] }), _jsxs("label", { className: "block", children: [_jsxs("span", { className: "label", children: ["Type ", _jsx("code", { className: "font-mono", children: agent || "<agent>" }), " to confirm"] }), _jsx("input", { className: "input w-full font-mono", value: confirmName, onChange: (e) => setConfirmName(e.target.value), placeholder: agent })] }), _jsxs("div", { className: "flex justify-end gap-2", children: [_jsx("button", { className: "btn", onClick: onClose, children: "Cancel" }), _jsx("button", { className: "btn btn-danger", disabled: !canSave || saving, onClick: submit, children: saving ? "Creating…" : "Create grant" })] })] }));
}
function DetectionRollup({ rollup }) {
    const buckets = Object.entries(rollup?.by_class ?? {}).sort((a, b) => b[1] - a[1]);
    const max = buckets.length > 0 ? Math.max(...buckets.map(([, n]) => n)) : 0;
    return (_jsxs("div", { className: "panel p-4", children: [_jsxs("div", { className: "flex items-center justify-between mb-3", children: [_jsx("h3", { className: "font-semibold", children: "Recent detections (last 24h)" }), _jsxs("span", { className: "text-xs text-spark-muted", children: ["total events: ", rollup?.total ?? 0] })] }), buckets.length === 0 ? (_jsx("p", { className: "text-sm text-spark-muted", children: "No guardrail events in the window." })) : (_jsx("table", { className: "w-full text-sm", children: _jsx("tbody", { children: buckets.map(([cls, n]) => (_jsxs("tr", { className: "border-t border-spark-border", children: [_jsx("td", { className: "py-1 font-mono w-56", children: cls }), _jsx("td", { children: _jsx("div", { className: "h-2 rounded bg-spark-accent/40", style: { width: `${max ? (n / max) * 100 : 0}%` } }) }), _jsx("td", { className: "text-right tabular-nums w-16", children: n })] }, cls))) }) }))] }));
}
